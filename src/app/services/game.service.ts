...
export class GameService {
  ...
  // Verificar level up (até nível 12, 10 XP por nível)
  ...
  private save(): void {
    localStorage.setItem('anthony-geografia-player', JSON.stringify(this.player));
  }

  // Carregar do localStorage
  load(): void {
    const data = localStorage.getItem('anthony-geografia-player');
    if (data) {
      try {
